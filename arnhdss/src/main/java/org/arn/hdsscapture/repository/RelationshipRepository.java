...
public interface RelationshipRepository extends JpaRepository <Relationship, String> {
	...
	@Query(nativeQuery = true, value ="SELECT a.uuid,aIsToB,a.endDate,a.endType,concat(f.firstName,' ',f.lastName) as fw_uuid,concat(b.firstName,' ',b.lastName,' ',COALESCE(otherName, '')) as individualA_uuid,individualB_uuid,a.insertDate,lcow,mar, \r\n"
	...
	@Query(nativeQuery = true, value ="SELECT a.uuid,aIsToB,a.endDate,a.endType,concat(f.firstName,' ',f.lastName) as fw_uuid, "
			...
			+ "INNER JOIN residency c on b.uuid=c.individual_uuid INNER JOIN location d on c.location_uuid=d.uuid\r\n"
			+ "INNER JOIN fieldworker f on a.fw_uuid=f.fw_uuid\r\n"
			+ "where a.insertDate > (SELECT r.startDate from round r ORDER BY r.roundNumber DESC limit 1) AND c.endType=1")
	List<Relationship> findItem();
}
