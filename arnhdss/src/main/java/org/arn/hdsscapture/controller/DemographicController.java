...
public class DemographicController {
    ...
    public DataWrapper<Demographic> saveAll(@RequestBody DataWrapper<Demographic> data) {
        List<Demographic> savedRecords = new ArrayList<>();
        List<Demographic> errorRecords = new ArrayList<>();

        try {
            // Iterate over each Demographic record to validate/save individually
            for (Demographic record : data.getData()) {
                try {
                    // Try saving individual record
                    Demographic saved = repo.save(record);
                    ...
                    String residencyUuid = 
                    	    ...
                    	    "Phone: " + (record.getPhone1() != null ? record.getPhone1() : "Unknown");
                    String fw = record.getFw_uuid();
                    String tb = "Demographic";
            ...
            DataWrapper<Demographic> response = new DataWrapper<>();
